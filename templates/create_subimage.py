...
def main():
    parser = argparse.ArgumentParser(
        description="Extract Stokes parameters (I, Q, U) from a full Stokes cube",
...
Examples:
  ...
  ./create_subimage.py --inputcube mycube_IQUV.fits --crop "[800, 800]" --pointing "[3072, 3072]"
    ...
    parser.add_argument('--inputcube', required=True,
                        help='Path to input full Stokes cube (must contain I, Q, U)')
    ...
    make_empty_image(filename, crop, pointing, mode="stokesQ")
    fill_cube_with_images(filename, crop, pointing, mode="stokesQ")
    make_empty_image(filename, crop, pointing, mode="stokesU")
    fill_cube_with_images(filename, crop, pointing, mode="stokesU")
